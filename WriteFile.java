...
public class WriteFile {
		...
		try {
			...
			for( int targetV = Constances.INITIALV; targetV <= Constances.MAXV; targetV += Constances.STEPV){
				for(int alg = 0; alg < Constances.ALGORITHM; alg++){
					pw.write(String.valueOf(data[targetV/Constances.STEPV - 1][alg][0]));
					pw.write('\t');
					pw.write(String.valueOf(data[targetV/Constances.STEPV - 1][alg][1]));
					pw.write("\r\n");
	...
	public void rawDateWriter(String path, double [][][] data) {
		...
		try {
			...
			for(int targetV = Constances.INITIALV; targetV <= Constances.MAXV; targetV += Constances.STEPV){
				pw.write("\r\n");
				pw.write("v=" + String.valueOf(targetV));
				pw.write("\r\n");
				for(int alg = 0; alg < Constances.ALGORITHM; alg++){
					for(int loop = 0; loop < Constances.LOOP; loop ++){
						pw.write(String.valueOf(data[targetV/Constances.STEPV-1][alg][loop]));
						pw.write('\t');
					}
					pw.write("\r\n");
				}				
